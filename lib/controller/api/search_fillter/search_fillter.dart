...
Future<BookDtoListApiResult> searchAndFillter(
    {required BuildContext context, required BookSearchDto? body}) async {
  final api = Tikonline.create(interceptors: [TokenInterceptor('accessToken')]);

  final postResult = await api.apiV1BookSearchandFilterPost(body: body);
  print(postResult);
  ...
  print(response.data);
  context.read<SearchFillterState>().getList(value: response.data!);
