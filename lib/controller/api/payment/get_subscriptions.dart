...
Future<SubscriptionDtoListApiResult> getSubscriptions({
  required BuildContext context,
}) async {
  final api = Tikonline.create(interceptors: [TokenInterceptor('accessToken')]);

  final postResult = await api.apiV1PaymentSubscriptionListGet();
  print(postResult);
  ...
  final response =
      SubscriptionDtoListApiResult.fromJson(postResult.body!.toJson());
  print(response.data);
  context.read<SubscriptionsState>().getSubscriptions(value: response.data!);
